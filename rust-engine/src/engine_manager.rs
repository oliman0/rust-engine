...
impl<'a> Engine<'a> {
    ...
    pub fn new_scene(&mut self, build: fn() -> (Vec<Mesh>, Vec<UIElement>), update: fn(&mut Scene, &InputHandler, f32), pos: nalgebra_glm::Vec3, col: nalgebra_glm::Vec4) {
        self.scenes.push(scene(build, update, pos, col))
    }
    ...
    pub fn input_handler(&self) -> &InputHandler { &self.input_handler }
}
